...
struct proxy_ops {
    ...
    void (*evctl)(struct proxy *proxy, unsigned int events, int enable);
...
*/
static inline void proxy_evctl(struct proxy *proxy, unsigned int events,
                               int enable)
{
    proxy->ops->evctl(proxy, events, enable);
}
